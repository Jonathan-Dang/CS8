...
public:
    ...
    void Intersection(Set<T>& rhs, Set<T>& outs);//return a Set that is only the duplicates between the two trees
    void Union(Set<T>& rhs, Set<T>& outs);       //return a Set that is the combination of the two, no dups
...
private:
    ...
    //Union Intersection Helper Function
    void Vectorize(vector<T>& v);                   //Outputs a copy of all the tree's data to a Vector
...
template<class T>
inline Set<T>::Set() : dups_ok(false), data_count(0), child_count(0)
{
...
template<class T>
inline void Set<T>::print_tree(int level, ostream& outs) const
{
    //print last suSet
    if (!is_leaf())
    {
...
template<class T>
inline bool Set<T>::is_valid(bool main_root)
{
    if (main_root)
    {
...
template<class T>
inline void Set<T>::Vectorize(vector<T>& v)
{
    if (!is_leaf())
    {
        subset[child_count - 1]->Vectorize(v);
    }
    ...
    for (int i = data_count - 1; i >= 0; i--)
    {
        ...
        if (!is_leaf())
            subset[i]->Vectorize(v);
    }
}

template<class T>
inline void Set<T>::loose_insert(const T& entry)
{
...
template<class T>
inline void Set<T>::fix_excess(int i) //Child count isn't updated | why is the subset of 0 and 1 of subset[0] have 20 children 
{
    //Create a new Child
    Set<T>* fix = new Set<T>;
...
template<class T>
inline void Set<T>::grow_tree()
{
    //Make a new Set node
    Set<T>* grow = new Set<T>;
...
template<class T>
inline void Set<T>::loose_remove(const T& entry)
{
    int i = first_ge(data, data_count, entry);
    bool found = (i < data_count&& data[i] == entry);
...
template<class T>
inline void Set<T>::fix_shortage(int i)
{
...
template<class T>
inline void Set<T>::insert(const T& entry)
{
...
template<class T>
inline void Set<T>::remove(const T& entry)
{
    loose_remove(entry);
    if (data_count < MINIMUM)
    {
...
template<class T>
inline void Set<T>::copy_tree(const Set<T>& other)
{
    //Copy at this Level
...
template<class T>
inline bool Set<T>::contains(const T& entry)
{
    int i = first_ge(data, data_count, entry);
    bool found = (i < data_count&& data[i] == entry);
...
template<class T>
inline T& Set<T>::get(const T& entry)
{
    if (!contains(entry))
        insert(entry);
...
template<class T>
inline T& Set<T>::get_existing(const T& entry)
{
    int i = first_ge(data, data_count, entry);
    bool found = (i < data_count&& data[i] == entry);
...
template<class T>
inline T* Set<T>::find(const T& entry)
{
    int i = first_ge(data, data_count, entry);
    bool found = (i < data_count&& data[i] == entry);
...
template<class T>
inline const T& Set<T>::get(const T& entry) const
{
    int i = first_ge(data, data_count, entry);
    bool found = (i < data_count&& data[i] == entry);
...
template<class T>
inline void Set<T>::Intersection(Set<T>& rhs, Set<T>& outs)
{
    if (!outs.empty())
        outs.clear_tree();

    vector<T> rhs_data;
    rhs.Vectorize(rhs_data);

    for (int i = 0; i < rhs_data.size(); i++)
    {
        if (contains(rhs_data[i]))
            outs.insert(rhs_data[i]);
    }
}

template<class T>
inline void Set<T>::Union(Set<T>& rhs, Set<T>& outs)
{
    if (!outs.empty())
        outs.clear_tree();

    vector<T> rhs_data, lhs_data;
    rhs.Vectorize(rhs_data);
    Vectorize(lhs_data);
    ...
    {
        outs.insert(lhs_data[i]);
    }
}

template<class T>
inline void Set<T>::remove_max(T& entry)
{
    if (!is_leaf())
    {
...
template<class T>
inline void Set<T>::rotate_left(int i) //Right Child has 2
{
    //Hold Assertions
    assert(0 < i);
...
template<class T>
inline void Set<T>::rotate_right(int i) //Left Child has 2
{
    //Hold Assertions
...
template<class T>
inline void Set<T>::merge_with_next_subset(int i)
{
